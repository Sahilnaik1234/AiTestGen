...
class TestInventoryManager(unittest.TestCase):
    ...
    def test_load_data(self):
        with open("test_inventory.json", 'w') as f:
            json.dump({"item1": {"name": "Item 1", "quantity": 10, "price": 10.0}}, f)
        self.manager.load_data()
        self.assertEqual(self.manager.items, {"item1": {"name": "Item 1", "quantity": 10, "price": 10.0}})

    def test_save_data(self):
        self.manager.items = {"item1": {"name": "Item 1", "quantity": 10, "price": 10.0}}
        self.manager.save_data()
        with open("test_inventory.json", 'r') as f:
            self.assertEqual(json.load(f), {"item1": {"name": "Item 1", "quantity": 10, "price": 10.0}})

    def test_add_item(self):
        result, message = self.manager.add_item("item1", "Item 1", 10, 10.0)
        self.assertTrue(result)
        self.assertEqual(message, "Item added successfully")
        self.assertEqual(self.manager.items, {"item1": {"name": "Item 1", "quantity": 10, "price": 10.0, "last_updated": str(datetime.datetime.now())}})

    def test_add_item_duplicate(self):
        self.manager.add_item("item1", "Item 1", 10, 10.0)
        result, message = self.manager.add_item("item1", "Item 1", 10, 10.0)
    ...
    def test_get_item(self):
        ...
        self.assertEqual(item, {"name": "Item 1", "quantity": 10, "price": 10.0, "last_updated": str(datetime.datetime.now())})
    ...
    def test_calculate_total_value(self):
        self.manager.add_item("item1", "Item 1", 10, 10.0)
        self.manager.add_item("item2", "Item 2", 20, 20.0)
        total_value = self.manager.calculate_total_value()
        self.assertEqual(total_value, 10 * 10 + 20 * 20)

    def test_find_items_below_threshold(self):
        self.manager.add_item("item1", "Item 1", 10, 10.0)
        self.manager.add_item("item2", "Item 2", 5, 20.0)
    ...
    def test_bulk_update_prices(self):
        ...
        self.assertEqual(self.manager.items["item1"]["price"], 11.0)
        self.assertEqual(self.manager.items["item2"]["price"], 22.0)
    ...
    def test_generate_report(self):
        ...
        self.assertIn("ID: item1 | Name: Item 1 | Qty: 10 | Price: $10.00", report)
        self.assertIn("ID: item2 | Name: Item 2 | Qty: 20 | Price: $20.00", report)
        self.assertIn("Total Value: $500.00", report)
