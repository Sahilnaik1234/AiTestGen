...
describe('InventoryManager', () => {
    ...
    describe('releaseReservation', () => {
        ...
        it('should not release a reservation if quantity exceeds reserved stock', () => {
            inventoryManager.addStock('item1', 10);
            inventoryManager.reserveStock('item1', 5);
            expect(inventoryManager.releaseReservation('item1', 15)).toBe(false);
        });
    });
    ...
    describe('fulfillReservation', () => {
        ...
        it('should not fulfill a reservation if quantity exceeds reserved stock', () => {
            inventoryManager.addStock('item1', 10);
            inventoryManager.reserveStock('item1', 5);
            expect(inventoryManager.fulfillReservation('item1', 15)).toBe(false);
        });
    });
    ...
    describe('isLowStock', () => {
        it('should return true if the available stock is low', () => {
            inventoryManager.addStock('item1', 10);
            inventoryManager.reserveStock('item1', 8);
            expect(inventoryManager.isLowStock('item1')).toBe(true);
        });

        it('should return false if the available stock is not low', () => {
            inventoryManager.addStock('item1', 10);
            expect(inventoryManager.isLowStock('item1')).toBe(false);
        });

        it('should use the default threshold if not provided', () => {
            inventoryManager.addStock('item1', 10);
            inventoryManager.reserveStock('item1', 6);
            expect(inventoryManager.isLowStock('item1')).toBe(false);
        });

        it('should use the provided threshold', () => {
            inventoryManager.addStock('item1', 10);
            inventoryManager.reserveStock('item1', 6);
            expect(inventoryManager.isLowStock('item1', 3)).toBe(true);
        });
    });
