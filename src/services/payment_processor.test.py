...
class TestPaymentProcessor(unittest.TestCase):
    ...
    def test_refund_payment_expired_refund_window(self):
        processor = PaymentProcessor()
        amount = 100.0
        card_number = "4111111111111111"
        result = processor.process_payment(amount, card_number)
        transaction_id = result["transaction_id"]
        record = next((record for record in processor.transaction_log if record["transaction_id"] == transaction_id), None)
        record["timestamp"] = datetime.now() - timedelta(days=31)
